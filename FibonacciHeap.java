...
public class FibonacciHeap{
    ...
    */
    public int[] countersRep() {
    	...
    	int[] rankArray = new int[(int) Math.floor(Math.log(this.size())/Math.log(GOLDAN_RATIO))+1];  //creates the array
    ...
    */
    public void decreaseKey(HeapNode x, int delta) {  
		...
		if (x.getKey() >= x.parent.getKey()) {return;} //we don't need to cut
    ...
    */
    ...
    private HeapNode[] toBuckets(HeapNode curr) {
    	HeapNode[] buckets = new HeapNode[(int) Math.floor(Math.log(this.size())/Math.log(GOLDAN_RATIO))+1];
    	curr.prev.next = null;
    	HeapNode tmpCurr;
    	while (curr != null) {
    		tmpCurr = curr;
    ...
    */
    public class HeapNode{
    	 ...
    	 */
    	private void mark() {
    		if (this.isRoot()) {return;}  //check if the node is a root
    		this.marked = true;
    	}
